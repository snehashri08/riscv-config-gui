...
global csr_name, values, sub_field, window
...
def update_fields():
   ...
   if sub_field ==[]:
      for k in list(set(isa_yaml['hart0'][csr_name][rvxlen].keys()) - set(['fields'])):
...
def inner(csr):
  ...
  if sub_field == [] :
     rr= [[sg.Text(k), sg.InputText(isa_yaml['hart0'][csr][rvxlen][k], key='-'+csr+'_'+k+'-') ] for k in list(set(isa_yaml['hart0'][csr][rvxlen].keys()) - set(['fields', 'type'])) ]
     _type= [[sg.Text('type'), sg.InputText(eval(str(isa_yaml['hart0'][csr][rvxlen]['type']).replace('ordereddict','dict')), key='-'+csr+'_type-') ]]
  else:
    rr=[[sg.Column([[sg.Frame(sub, [[sg.Text(k), sg.InputText(isa_yaml['hart0'][csr][rvxlen][sub][k], key='-'+csr+'_'+sub+k+'-') ] for k in list(set(isa_yaml['hart0'][csr][rvxlen][sub].keys()) -set(['fields','type'])) ]+ [[sg.Text('type'), sg.InputText(eval(str(isa_yaml['hart0'][csr][rvxlen][sub]['type']).replace('ordereddict','dict')), key='-'+csr+'_'+sub+'type-') ]]) ]  for sub in sub_field] , pad=(0,0), scrollable=True, key = "Columnmmm", size=(400,500))]]
    _type= [[]]
  return [[sg.Text(k), sg.InputText(isa_yaml['hart0'][csr][k], key='-'+csr+'_'+k+'-') ] for k in list(set(isa_yaml['hart0'][csr].keys())-set(['rv32', 'rv64'])) ] + rr +_type
...
def page1(csr):
    ...
    col3 = sg.Column([[sg.Frame('Actions:',
                            [[sg.Column([[sg.Button('Back To Index'), sg.Button('Save'), ]],
